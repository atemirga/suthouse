...
export async function GET(req: NextRequest) {
  ...
  const from = url.searchParams.get('from') ? startOfDay(parseISO(url.searchParams.get('from')!)) : startOfMonth(new Date());
  const to = url.searchParams.get('to') ? endOfDay(parseISO(url.searchParams.get('to')!)) : endOfMonth(new Date());
