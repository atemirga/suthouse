import { buildReceivables, AGING_BUCKETS, type BucketKey } from '@/lib/reports/receivables';
...
export default async function ReceivablesPage({ searchParams }: Props) {
  ...
  const report = await buildReceivables({ asOf });
      ...
      <div className="flex items-baseline justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            На {format(report.asOf, 'dd.MM.yyyy')} · {report.totals.debtorCount} должников · разбивка по возрасту долга
          </p>
        </div>
      </div>
      ...
      <ReceivablesClient
        initialData={JSON.parse(JSON.stringify({
          ...
          rows: report.rows.map((r) => ({
            ...r,
            oldestDate: r.oldestDate?.toISOString() || null,
          })),
        }))}
